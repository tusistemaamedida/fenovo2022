        ...
        <div class="row">
            <div class="col-lg-12 col-xl-12">
                <div class="card card-custom gutter-b bg-transparent shadow-none border-0">
                    <div class=" border-bottom-dark px-0">
                        <div class="row mt-5 mb-5">
                            <div class="col-12">
                                <h4>Tienda :: {{ str_pad($store->cod_fenovo, 3, '0', STR_PAD_LEFT)  }} - {{ $store->description }}</h4>
                            </div>
                        </div>
                        <div class="row mt-5 mb-5">                        
                            <div class="col-6">
                                {{ $producto->cod_fenovo }} - {{ $producto->name }} - Unidad <span class=" text-danger">{{ $producto->unit_type }} </span>
                            </div>    
                        </div>
                    </div>
                </div>
...
<script>
    var table = jQuery('.yajra-datatable').DataTable({
        ...
        autoWidth: false,
        ajax: '{{ route('product.historial.tienda', ['product_id' => $producto->id, 'store_id' => $store->id] ) }}',
        columns: [
